...
volatile unsigned int RemapBadBlockFlag;

// (jhpark): bad block mapping
void HardCodingBadBlock(unsigned int ch, unsigned int way, unsigned int rowAddr)
{
	unsigned int dieNum, plane, block, phyBlock;
	dieNum = Pcw2VdieTranslation(ch, way);
	if(rowAddr >= LUN_1_BASE_ADDR)
	{
		plane = 1;
		block = (rowAddr - LUN_1_BASE_ADDR)/PAGES_PER_MLC_BLOCK;
		phyBlock = block + TOTAL_BLOCKS_PER_LUN;
	}
	else
	{
		plane = 0;
		block = (rowAddr - LUN_0_BASE_ADDR)/PAGES_PER_MLC_BLOCK;
		phyBlock = block;
	}

	phyBlockMapPtr->phyBlock[dieNum][phyBlock].bad = BLOCK_STATE_BAD;
	xil_printf("Set bad flag ch %d way %d plane %d block %d rowAddr %x\r\n",ch, way, plane, block, rowAddr);
}
...
void InitSliceMap()
{
	...
	for(sliceAddr=0; sliceAddr<SLICES_PER_SSD ; sliceAddr++)
	{
		...
		logicalSliceMapPtr->logicalSlice[sliceAddr].stream_id = -1;
	}
}
...
void RemapBadBlock()
{
	...
	//debug
	//for(blockNo=USER_BLOCK_START_NO ; blockNo<USER_BLOCKS_PER_LUN ; blockNo++)
...
void EraseUserBlockSpace()
{
	...
	//debug
	//for(blockNo=USER_BLOCK_START_NO ; blockNo<USER_BLOCKS_PER_DIE ; blockNo++)
...
void InitBlockDieMap()
{
	...
	// (jhpark): bad block mapping
	// Read Transfer FAIL on     ch 0 way 4 rowAddr 700 / completion A5000001 statusReport 1C1C1
	// Read Transfer FAIL on     ch 0 way 4 rowAddr 500 / completion A5000001 statusReport 1C1C1
	//HardCodingBadBlock(0,4,0x700);
	//HardCodingBadBlock(0,4,0x500);
	HardCodingBadBlock(1,3,0x202300);
	HardCodingBadBlock(1,3,0x202301);
...
unsigned int AddrTransWrite(unsigned int logicalSliceAddr, unsigned int stream_id)
{
	unsigned int virtualSliceAddr;
	...
	if(logicalSliceAddr < SLICES_PER_SSD)
	{
		...
		// (jhpark): opt
		stream_id = logicalSliceMapPtr->logicalSlice[logicalSliceAddr].stream_id;
		virtualSliceAddr = FindFreeVirtualSlice(stream_id);
...
unsigned int FindFreeVirtualSlice(unsigned int stream_id)
{
	unsigned int currentBlock, virtualSliceAddr, dieNo;

	// step1. stream_id로 dieNo, blockNo 가져오기
	if (stream_id != -1) {

		// no striping version
		if (stream_controller[stream_id].dieNo == -1) {
			 stream_controller[stream_id].dieNo = FindDieForFreeSliceAllocation();
		}
		if (stream_controller[stream_id].blockNo == -1) {
			dieNo = stream_controller[stream_id].dieNo;
			stream_controller[stream_id].blockNo = virtualDieMapPtr->die[dieNo].currentBlock;
		}

		dieNo = stream_controller[stream_id].dieNo;
		currentBlock = stream_controller[stream_id].blockNo;
	...
	if(virtualBlockMapPtr->block[dieNo][currentBlock].currentPage > USER_PAGES_PER_BLOCK) {
		 printf("page write error! (%u) check here %u %u stream-id: %d\n"
				 , virtualBlockMapPtr->block[dieNo][currentBlock].currentPage
				 , dieNo, currentBlock, stream_id);
	}

	// step2. 현재 stream에 할당된 block full인지 확인
	if(virtualBlockMapPtr->block[dieNo][currentBlock].currentPage == USER_PAGES_PER_BLOCK)
	{
		...
		// stream block 갱신하기
		stream_controller[stream_id].dieNo = FindDieForFreeSliceAllocation();
		stream_controller[stream_id].blockNo = virtualDieMapPtr->die[stream_controller[stream_id].dieNo].currentBlock;
	}
	else if(virtualBlockMapPtr->block[dieNo][currentBlock].currentPage > USER_PAGES_PER_BLOCK)
		assert(!"[WARNING] Current page management fail [WARNING]");
	...
	if(virtualBlockMapPtr->block[dieNo][currentBlock].currentPage == USER_PAGES_PER_BLOCK) {
		printf("page write will return error! (%u) check here %u %u stream-id: %d\n"
				 , virtualBlockMapPtr->block[dieNo][currentBlock].currentPage
				 , dieNo, currentBlock, stream_id);
	}
	virtualBlockMapPtr->block[dieNo][currentBlock].currentPage++;
	...
	stream_controller[stream_id].pageCnt++;
	sliceAllocationTargetDie = FindDieForFreeSliceAllocation();
	dieNo = sliceAllocationTargetDie;
...
void InvalidateOldVsa(unsigned int logicalSliceAddr)
{
	...
	if(virtualSliceAddr != VSA_NONE)
	{
		...
		logicalSliceMapPtr->logicalSlice[logicalSliceAddr].stream_id = -1;
