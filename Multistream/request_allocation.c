...
void PutToSliceReqQ(unsigned int reqSlotTag)
{
	...
	AllocateNvmeDmaCnt(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag);
}
...
void PutToNvmeDmaReqQ(unsigned int reqSlotTag)
{
	...
	GetTimeHostReqNvmeDmaStart(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag);
}
...
void SelectiveGetFromNvmeDmaReqQ(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.forceUnitAccess == REQ_OPT_FORCE_UNIT_ACCESS_ON)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_RxDMA)
			CompleteRxforNvmeIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
		if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_TxDMA)
			CompleteTxforNvmeIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
	}

	GetTimeHostReqNvmeDmaEnd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag);

	PutToFreeReqQ(reqSlotTag);
	ReleaseBlockedByBufDepReq(reqSlotTag);

	UpdatePerformanceCounterForCompletedNvmeDmaReq(reqSlotTag);
	UpdatePerformanceCounterForCompletedFuaReq(reqSlotTag);
}
...
void GetFromNandReqQ(unsigned int chNo, unsigned int wayNo, unsigned int reqStatus, unsigned int reqCode)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.forceUnitAccess == REQ_OPT_FORCE_UNIT_ACCESS_ON)
		CompleteNandReqforNvmeIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqCode, reqStatus);

	GetTimeBufferEvictNandReqEnd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag);
