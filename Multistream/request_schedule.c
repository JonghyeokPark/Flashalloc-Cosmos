...
void SchedulingNandReqPerCh(unsigned int chNo)
{
	...
	if(waitWayCnt != USER_WAYS)
		...
		{
			UpdatePerformanceCounterForIdleChannel(chNo);
			...
			UpdatePerformanceCounterForActiveChannel(chNo);
...
void IssueNandReq(unsigned int chNo, unsigned int wayNo)
{
	...
	GetTimeBufferEvictNandReqStart(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag);
...
unsigned int GenerateNandRowAddr(unsigned int reqSlotTag)
{
	...
	if(RemapBadBlockFlag == 1)
	{
		xil_printf("GenPPA reqSlotTag %d ch %d way %d plane %d blk %d page %d row %x buffEntry %d\r\n",
			reqSlotTag, Vdie2PchTranslation(dieNo), Vdie2PwayTranslation(dieNo),
			lun, tempBlockNo, tempPageNo, rowAddr, reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry);
	}
#endif
