...
#if 1 /*woonhak, at this point we should have doublewrite file */
	/* special file for doublewrite buffer */
	if (srv_doublewrite_file)
	{
		os_file_t doublewrite_file;
		ibool ret;
		char	name[10000];
		...
		//doublewrite_file = os_file_create(innodb_data_file_key, name, OS_FILE_CREATE,
		/*woonhak, always make a new file */ 
		doublewrite_file = os_file_create(innodb_data_file_key, name, OS_FILE_OVERWRITE,
				OS_FILE_NORMAL,
				OS_DATA_FILE, &ret);
...
#if 0 /*woonhak, Add Set StreamID for double write file*/
		ret = os_file_set_streamid(
				name, doublewrite_file, 2, (off_t) ((TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 16) << UNIV_PAGE_SIZE_SHIFT));
#endif

		{
			...
			fprintf(stderr,
					...
					name,
					(ulong) ((TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2)
						>> (20 - UNIV_PAGE_SIZE_SHIFT)));
			...
			ret = os_file_set_size(
					name, doublewrite_file,
					(ulong) ((TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2) << UNIV_PAGE_SIZE_SHIFT));
      ...
      // (jhpark): truncate DWB file size as 2MB
      //ret = os_file_truncate(name, doublewrite_file, (2*1024*1024));
      //if (!ret) {
      //  fprintf(stderr,
      //      "InnoDB: Error in creating %s:\n", name);
      //}
		...
		ret = os_file_close(doublewrite_file);
		ut_a(ret);
		...
		fil_node_create(name, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2, TRX_DOUBLEWRITE_SPACE, FALSE);
...
#if 1 /*woonhak, need to create new doublewrite first */

	if (srv_doublewrite_file) {
		mtr_start(&mtr);
		fsp_header_init(TRX_DOUBLEWRITE_SPACE, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2, &mtr);
		mtr_commit(&mtr);

		trx_sys_dblwr_create(TRX_DOUBLEWRITE_SPACE);
	}                                                                                    
#endif
