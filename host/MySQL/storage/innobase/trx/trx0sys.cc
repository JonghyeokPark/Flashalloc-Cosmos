...
#if 0 /*woonhak, to debug */
	ib_logf(IB_LOG_LEVEL_INFO,
		"TRX_SYS_PAGE_NO:%ld",buf_block_get_page_no(block)); 
#endif
...
#if 1 /*woonhak, for separate doublewrite */
/*****************************************************************//**
Creates dummy of the file page for the transaction system. */
static
void
trx_sysf_dblwr_create(
/*==================*/
	mtr_t*	mtr)
{
	buf_block_t*	block;
	...
	/* Create the trx sys file block in a new allocated file segment */
	block = fseg_create(TRX_DOUBLEWRITE_SPACE, 0, TRX_SYS + TRX_SYS_FSEG_HEADER,
			    mtr);
	buf_block_dbg_add_level(block, SYNC_TRX_SYS_HEADER);

	fprintf(stderr, "Doublewrite sys pageno : %lu, TRX_SYS_PAGE_NO:%d\n", buf_block_get_page_no(block), TRX_SYS_PAGE_NO);
	ut_a(buf_block_get_page_no(block) == TRX_SYS_PAGE_NO);
...
void
trx_sys_dblwr_create(ulint space)
/*================*/
{
	mtr_t mtr;
