...
#if 1 /*woonhak, Add Set StreamID for double write file*/
      // (jhpark): call sallocate for dwb file
      if (strstr(node->name, "dblwr") != NULL) { 
        int dwb_fd = (int)(node->handle);
        if (dwb_fd != -1) {
          struct fiemap* block_map = read_fiemap(dwb_fd);
          if (block_map) {
            fprintf(stderr, "[INFO] DWB file sallocate called at open file!!!\n");
          ...
          fprintf(stderr, "[DEBUG] Can not open DWB file (%s)\n", srv_doublewrite_file);
			...
			//os_file_set_streamid(node->name, node->handle, 3, node->size);
#endif
	}
