...
#if 1 /*woonhak, new header file for streamid*/
	...
	os_offset_t	size)		/*!< in: size preserved in bytes */
{
	...
	if (size >= size_bytes && strstr(pathname, "dblwr") == NULL) {
    fprintf(stderr, "[DEBUG] truncate not working!! (%s)\n", pathname);
		return(TRUE);
	}
...
#if 1 /*woonhak, add this for set streamid*/
ibool
os_file_set_streamid(
/*===============*/
  const char *name,
	os_file_t	file,  /*!< in, own: handle to a file */
	off_t 	streamid,  /*!< in, streamid, should be < 8 */
	uint64_t file_size	)	
{
	int fd;
	fd = (int) file;

	ut_ad(streamid < FADV_STREAMID_MAX); 

	/*set streamid for file*/
  // org-woonhak version
	//if(posix_fadvise(fd, streamid, (off_t)file_size, POSIX_FADV_STREAMID))
  /*
  if (fcntl(fd, F_SET_FILE_RW_HINT, &streamid) < 0)
	{
		fprintf(stderr, "SET STREAMID FOR %s FAIL : %s\n", 
				name == NULL ? "Not Passed" : name,
				strerror(errno));
		return FALSE;
	}
  */
