...
int submit_multiple_allocate(char *buffer, uint32_t pairs)
{
...
int submit_ioctl(int opcode, char *query, __u32 data_len, __u32 lba, int option, char **result)
{
	__u32 dword13;
	int err;
	void *ioctl_buf = NULL;
	...
	if (err = posix_memalign(&ioctl_buf, getpagesize(), data_len))
	{
