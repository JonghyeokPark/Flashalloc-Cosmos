...
bool PosixStreamedWrite(int fd, int write_hint_, const char* buf, size_t nbyte) {
  const size_t kLimit1Gb = 1UL << 30;
  ...
  // init Cosmos+ Board
  int cosmos_fd = open("/dev/nvme1n1", O_RDWR | O_DIRECT);
  if (fd < 0) {
    fprintf(stderr, "Comsmos+ Board Open Error!\n");
    return -1;
  }

  // get startLba of write
  struct fiemap* block_map = read_fiemap(fd);
  if (!block_map) {
    fprintf(stderr,"Current file does not have extents\n");
  }
  uint64_t slba = block_map->fm_extents[0].fe_physical / 4096;

  while (left != 0) {
    size_t bytes_to_write = std::min(left, kLimit1Gb);
    //ssize_t done = write(fd, src, bytes_to_write);

    mstream_write(cosmos_fd, write_hint_, slba, bytes_to_write/4096, src);
    ssize_t done = bytes_to_write;
...
bool PosixPositionedStreamedWrite(int fd, int write_hint, const char* buf, size_t nbyte, off_t offset) {
  const size_t kLimit1Gb = 1UL << 30;
  ...
  while (left != 0) {
    ...
    //ssize_t done = pwrite(fd, src, bytes_to_write, offset);
    mstream_write(cosmos_fd, write_hint, slba, bytes_to_write/4096, src);
    ssize_t done = bytes_to_write;
...
void PosixWritableFile::SetLevel(uint64_t level) {
  level_ = level;
}
...
// Suppress Valgrind "Unimplemented functionality" error.
    ...
    // get startLba of write
    struct fiemap* block_map = read_fiemap(fd_);
    if (!block_map) {
      fprintf(stderr,"Current file does not have extents\n");
    }

    // debug
    fprintf(stderr, "[debug] fd: %d set stream: write_hint!: %d num_extent: %d length: %d "
       , fd_, hint, block_map->fm_mapped_extents
        , block_map->fm_extents[0].fe_length);

    // todo (opt)
    allocsize_ = 73819750;
    for (__u32 i = 0; i <= block_map->fm_mapped_extents; i++) {
      mstream_set_stream(cosmos_fd, hint ,
              block_map->fm_extents[i].fe_physical / 4096, allocsize_/4096); 
    }
    */
...
IOStatus PosixWritableFile::Allocate(uint64_t offset, uint64_t len,
  ...
  if (alloc_status == 0) {
    ...
    // debug
    //fprintf(stderr, "[debug] set stream: write_hint!: %d level: %d\n", write_hint_, level_);

    for (__u32 i = 0; i < block_map->fm_mapped_extents; i++) {
      mstream_set_stream(cosmos_fd, write_hint_ ,
              block_map->fm_extents[i].fe_physical / 4096, len/4096); 
    }
