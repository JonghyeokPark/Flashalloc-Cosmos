...
Status BuildTable(
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    ...
    compression_opts_for_flush.zstd_max_train_bytes = 0;
    {
      ...
      // (jhpark): set write_hint according to level
      if (level == 0) {
        write_hint = Env::WLTH_SHORT;
      } else if (level == 1) {
        write_hint = Env::WLTH_MEDIUM;
      } else if (level == 2 || level ==3) {
        write_hint = Env::WLTH_LONG;
      } else if (level == 4 || level ==5) {
        write_hint = Env::WLTH_EXTREME;
      } else {
        write_hint = Env::WLTH_NOT_SET;
      }
