...
static void *prepare_timing_buf (unsigned int len)
{
	...
	buf = static_cast<__u8*>(mmap(NULL, len, PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE, -1, 0));
	if (buf == MAP_FAILED) {
...
static void time_cache (int fd)
{
	...
	buf = static_cast<char*>(prepare_timing_buf(TIMING_BUF_BYTES));
	if (!buf)
		return;
...
do_set_security (int fd)
{
	...
	r = static_cast<struct hdio_taskfile*>(malloc(sizeof(struct hdio_taskfile) + 512));
	if (!r) {
		err = errno;
		perror("malloc()");
		exit(err);
...
static void get_identify_data (int fd)
{
	...
	id = static_cast<__u16*>((void *)(args + 4));
	for (i = 0; i < 0x100; ++i)
		__le16_to_cpus(&id[i]);
}
...
{
	static __u8 args[4+512];
	__u16 *dco = static_cast<__u16*>((void *)(args + 4));
	int i;
...
static int do_make_bad_sector (int fd, __u64 lba, const char *devname)
{
	...
	r = static_cast<struct hdio_taskfile *>(malloc(sizeof(struct hdio_taskfile) + 520));
	if (!r) {
		err = errno;
		perror("malloc()");
		return err;
...
static void do_trim_sector_ranges (int fd, const char *devname, int nranges, struct sector_range_s *sr)
{
	...
	data = static_cast<__u64*>(mmap(NULL, data_bytes, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0));
	if (data == MAP_FAILED) {
		err = errno;
	...
	if (do_IDentity) {
		...
		if (id) {
			if (do_IDentity == 2)
				...
				identify(static_cast<short unsigned int*>((void *)id));
		}
	}
	if (get_lookahead) {
		get_identify_data(fd);
...
get_longarg (void)
{
	...
	if (0 == strcasecmp(name, "verbose")) {
		...
		trim_sector_ranges = static_cast<sector_range_s*>(malloc(sizeof(struct sector_range_s) * max_ranges));
